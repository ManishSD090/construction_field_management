...
├── company_settings.dart         # 🆕 NEW - Company + CompanySettings
├── client.dart                   # 🔄 REFACTORED - Client model
├── project_related.dart          # 🆕 NEW - Project + ProjectSettings + Milestone
...
│
└── [LEGACY - Keep for now]
    ├── project.dart              # ⚠️  Legacy (keep for backward compatibility)
    ├── company.dart              # ⚠️  Legacy (keep for backward compatibility)
    ├── user.dart                 # ⚠️  Legacy (can be deprecated)
    └── ... other legacy files
```
