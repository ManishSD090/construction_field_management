...
class _AddStaffScreenState extends ConsumerState<AddStaffScreen> {
  ...
  // 🚨 NEW: Load saved staff when screen opens
  @override
  void initState() {
    super.initState();
    _loadLocalStaff();
  ...
  // 🚨 NEW: Fetch saved staff from phone storage
      ...
      try {
        ...
        setState(() {
          ...
          _localStaffList.addAll(decodedData.map((data) => Worker(
            ...
            dailyWageRate: (data['dailyWageRate'] ?? 600).toDouble(),
          )));
        });
      } catch (e) {
  ...
  // 🚨 NEW: Save staff to phone storage so it survives hot restarts
  Future<void> _saveLocalStaff() async {
    final prefs = await SharedPreferences.getInstance();
    final String encodedData = jsonEncode(
      _localStaffList.map((w) => {
        'id': w.id,
        'name': w.name,
        'workerId': w.workerId,
  ...
  void _showAddStaffDialog() async {
    ...
    final staffFromDB = await ref.read(workerControllerProvider.notifier).fetchSystemStaff();
    ...
    final List<String> roleOptions = ['Site Engineer', 'Supervisor', 'Project Manager', 'Architect', 'Company Administrator', 'Staff'];
      ...
      showDialog(
        ...
        builder: (context) => StatefulBuilder(
          builder: (context, setDialogState) => AlertDialog(
            ...
            title: const Center(child: Text("Add Staff", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18))),
                ...
                children: [
                  ...
                  DropdownButtonFormField<Worker>(
                    ...
                    decoration: InputDecoration(
                      ...
                      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: const BorderSide(color: AppColors.primaryBlue)),
                    ),
                    icon: const Icon(Icons.keyboard_arrow_down, color: AppColors.primaryBlue),
                    items: staffFromDB.map((w) => DropdownMenuItem(value: w, child: Text(w.name ?? 'Unknown'))).toList(),
                    onChanged: (Worker? val) {
                      if (val != null) {
                        setDialogState(() {
                          localSelectedWorker = val;
                          String dbRole = val.designation ?? '';
                          if (roleOptions.contains(dbRole)) {
                            localSelectedRole = dbRole;
                          } else if (dbRole.isNotEmpty) {
                            roleOptions.add(dbRole);
                            localSelectedRole = dbRole;
                          }
                        });
                      }
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    onChanged: (val) => setDialogState(() => localSelectedRole = val),
                  ),
                  const SizedBox(height: 16),
                  
                  const Text("Aadhar card", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Colors.black87)),
                  ...
                  Align(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      icon: Icon(_aadharPath == null ? Icons.description_outlined : Icons.check_circle, 
                          color: AppColors.primaryBlue, size: 18),
                      label: Text(_aadharPath == null ? "Select file" : "File Selected", style: const TextStyle(color: AppColors.primaryBlue, fontWeight: FontWeight.bold)),
                  ...
                  const SizedBox(height: 6),
                  Text("Upload a jpeg, jpg, png, pdf no larger than 10 MB", style: TextStyle(color: Colors.grey.shade600, fontSize: 10)),
                  
                  const SizedBox(height: 24),
                  ElevatedButton(
                    onPressed: () {
                      if (localSelectedWorker != null && localSelectedRole != null) {
                        setState(() {
                          // Prevent duplicates from being added
                          if (!_localStaffList.any((w) => w.id == localSelectedWorker!.id)) {
                        ...
                        _saveLocalStaff(); // 🚨 Trigger save instantly!
                        _aadharPath = null;
                        Navigator.pop(context); 
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please select a name and role")));
                      }
                    },
  ...
  void _confirmDelete() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text("Are you sure you want to remove ${_selectedStaffIds.length} selected person(s)?"),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () {
              Navigator.pop(ctx); 
              setState(() {
                _localStaffList.removeWhere((local) => _selectedStaffIds.contains(local.id));
                _selectedStaffIds.clear();
                _isSelectionEnabled = false;
              });
              _saveLocalStaff(); // 🚨 Update storage when removed
            },
        ...
        children: [
          ...
          Expanded(
            child: workersAsync.when(
              data: (workers) {
                ...
                // Combine any DB staff with the ones saved in SharedPreferences
                List<Worker> combinedStaff = [...dbStaff, ..._localStaffList];

                if (combinedStaff.isEmpty) {
                   return const Center(child: Text("No staff assigned yet. Add one!", style: TextStyle(color: Colors.grey)));
                }
          ...
          if (_isSelectionEnabled && _hasSelection) 
            workersAsync.maybeWhen(
              data: (workers) {
                var dbStaff = workers.where((w) => w.designation != null && w.designation!.toLowerCase() != 'worker').toList();
  ...
  Widget _buildListTitleRow() {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          const Text("Staff List/ Payroll", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
          IconButton(
            icon: Icon(
              _isSelectionEnabled ? Icons.check_box : Icons.check_box_outline_blank, 
              color: _isSelectionEnabled ? AppColors.primaryBlue : Colors.grey.shade600
            ),
            onPressed: () => setState(() {
  ...
  Widget _buildBottomActions(List<Worker> allStaff) {
    return Container(
      ...
      child: ElevatedButton(
        ...
        onPressed: () {
          final selectedStaff = allStaff.where((w) => _selectedStaffIds.contains(w.id)).toList();
          Navigator.pop(context, selectedStaff);
        },
        child: const Text("Add to the Project", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
      ),
    );
  }
}